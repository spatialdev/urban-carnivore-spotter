...
class App extends Component {
  componentDidMount() {
    ...
    NeighborhoodService.getAllNeighborhoods().then((allNeighborhoods) => {
      const allKnownNeighborhoods = allNeighborhoods.filter(
        (neighborhood) => neighborhood !== "Unknown"
      );
      return updateAllNeighborhoods(allKnownNeighborhoods);
    });
  }
  ...
  render() {
    ...
    const header =
      isMobile &&
      (history.location.pathname === "/reports/create" ||
        history.location.pathname ===
          "/tacoma/reports/create") ? null : isMobile ? (
        <Header />
      ) : (
        <DesktopHeader />
      );
    ...
    const footer =
      isMobile && history.location.pathname !== "/reports/create" ? (
        <Footer />
      ) : null;
    return (
      <div className="App">
